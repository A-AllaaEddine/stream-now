...
const MovieMeta = () => {
  ...
  useEffect(() => {
    if (movie && movie.type === "movie" && MovieMetas.length > 0) {
      dispatch(
        fetchMovieStreamsStart({ AddonUrl, AddonsData, type, decodedID })
      );
    }
  }, [movie]);
  ...
  useEffect(() => {
    ...
    if (movie && movie.videos && movie.videos.length > 0) {
      ...
      const seasonEpisoDes = Object.values(
        movie.videos.reduce((acc, item) => {
  ...
  const selectStream = (stream) => {
    var encodedStreamUrl = encodeURIComponent(stream.url).replaceAll(
      "%2F",
      "~2F"
    );
  ...
  const goToTrailer = () => {
    navigate(`/player/trailer/${movie.trailers[0].source}`);
  };
  return (
    <>
      {!isLoading ? (
        movie ? (
          ...
          >
            <div className="movie-details-image-container"></div>
              ...
              <div className="movie-details-info">
                <div className="movie-left-container">
                  ...
                  <div className="runtime-rating-releaseInfo-container">
                    {movie.imdbRating && (
                      <p className="movie-imdbRating">{movie.imdbRating}</p>
                    )}
                    {movie.runtime && (
                    ...
                    {movie.releaseInfo && (
                      <p className="movie-releaseInfo">{movie.releaseInfo}</p>
                    )}
                  </div>
