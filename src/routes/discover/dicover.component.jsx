...
const Discover = () => {
  ...
  useEffect(() => {
    ...
    setSelectedId("top");
    dispatch(
      fetchTypeCatalogsStart({ selectedAddonUrl, selectedType, selectedId })
    );
  }, []);
        ...
        <div className="types-subtypes-container">
          <div className="types-container">
            <nav className="types-nav">
              <button className="types-button" onClick={toogleTypesDropdown}>
                {`${selectedType ? `${selectedType}` : "Seelct Type"}`} ▼
              </button>
          ...
          <div className="SubTypes-container">
            <nav className="SubTypes-nav">
              <button
                className="SubTypes-button"
                onClick={toogleSubTypesDropdown}
              >
                {`${selectedSubType ? `${selectedSubType}` : "Seelct SubType"}`}{" "}
                ▼
              </button>
              <div
                className={`${subTypesToggled ? "toggled" : ""} SubTypes-list`}
              >
                {subTypes &&
                    ...
                    return (
                      <li
                        ...
                        onClick={() => {
                          ...
                          setSelectedAddonUrl(subtype.addonUrl);
                          setSelectedSubType(subtype.name);
                          toogleSubTypesDropdown();
                        }}
                        className="SubTypes-item"
                      >
                        {subtype.name}
                      </li>
                    );
              ...
              {TypeCatalogs &&
                TypeCatalogs.filter((_, idx) => idx > 2).map((movie) => {
                  return (
                    <MovieCard
                      key={movie.id}
                      movie={{ addonUrl: TypeCatalogs[2].addonUrl, ...movie }}
